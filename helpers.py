...
def apply_units(
    input_object: [int, float, str, list, dict], function_one
) -> [str, list, dict]:
...
def compound_interest(
    principal_amount: int, interest_rate: float, days_passed: int
) -> int:
    return int(
        math.floor(principal_amount * (1 + interest_rate) ** (days_passed / 365))
    )
...
# Dates
def day_to_date(
    n_days_passed: int, _start_date: datetime.date = datetime.date(1700, 1, 1)
) -> str:
...
# Parsing arguments
def split_args(input_string: str, split_string: str = " ") -> list:
    if input_string == "":
        return []
    else:
        output_list = input_string.lower().split(split_string)
        if input_string[-1] == " ":
            output_list.append("")
    return output_list
...
def table_aligned_print(
    column_names: list,
    column_aligns: list,
    row_keys: list,
    column_dicts: list,
    show_header: bool = True,
    show_row_keys: bool = True,
) -> None:
    max_column_widths = []
    for column_index, column_dict in enumerate(column_dicts):
        ...
        if not (len(column_dict) == len(row_keys)):
            raise ValueError(
                "Column {}'s dict is the wrong length.".format(column_index)
            )
        ...
        if show_header:
            max_column_widths.append(
                max(max_variable_width, len(column_names[column_index]))
            )
        else:
    ...
    if show_header:
        for column_index, column_name in enumerate(column_names):
            text += align_text(
                column_name, max_column_widths[column_index], "l"
            ) + blanks(1)
        print("".join(text))
    ...
    for key_index, key in enumerate(row_keys):
        ...
        for column_index, column_dict in enumerate(column_dicts):
            text += align_text(
                column_dict[key],
                max_column_widths[column_index],
                column_aligns[column_index],
            ) + blanks(1)
        print("".join(text))
...
def bar(
    value: int, out_of: int, units=str, label: str = "", total_length: int = 25
) -> None:
    if value < 0:
        value = 0
    elif value > out_of:
    ...
    for syntax_element in command_syntax:
        ...
        for required_attribute in ["name", "type", "required"]:
            if required_attribute not in syntax_element:
                raise ValueError(
                    "Missing the '{}' atribute.".format(required_attribute)
                )
        # check the 'valid_values' attribute is of the correct type
        if (
            "valid_values" in syntax_element
            and syntax_element["valid_values"] is not None
        ):
            if syntax_element["type"] == "string":
                if (
                    isinstance(syntax_element["valid_values"], list)
                    and syntax_element["valid_values"] != []
                ):
                    if all(
                        type(valid_value) is str
                        for valid_value in syntax_element["valid_values"]
                    ):
                        pass
                    else:
                        raise TypeError(
                            "Elements of the attribute 'valid_values' of the argument '{}' must be strings.".format(
                                syntax_element["name"]
                            )
                        )
                else:
                    raise TypeError(
                        "Attribute 'valid_values' of the argument '{}' must be a nonempty list.".format(
                            syntax_element["name"]
                        )
                    )
                ...
                if type(syntax_element["valid_values"]) is tuple:
                    if 2 <= len(syntax_element["valid_values"]) <= 3:
                        if (
                            type(syntax_element["valid_values"][0]) is int
                            or type(syntax_element["valid_values"][0]) is float
                        ) and (
                            type(syntax_element["valid_values"][1]) is int
                            or type(syntax_element["valid_values"][1]) is float
                        ):
                            if (
                                syntax_element["valid_values"][0]
                                <= syntax_element["valid_values"][1]
                            ):
                                if len(syntax_element["valid_values"]) == 3:
                                    if type(syntax_element["valid_values"][2]) is list:
                                        ...
                                        TypeError(
                                            "Attribute 'valid_values' of the argument '{}' must be of the form (min, max, [extra_values]) where extra_values is a list.".format(
                                                syntax_element["name"]
                                            )
                                        )
                            else:
                                raise ValueError(
                                    "Attribute 'valid_values' of the argument '{}' must have min value smaller than max value.".format(
                                        syntax_element["name"]
                                    )
                                )
                        else:
                            raise TypeError(
                                "Attribute 'valid_values' of the argument '{}' must be of the form (min, max, [extra_values]) where min and max are of the type int or float.".format(
                                    syntax_element["name"]
                                )
                            )
                    else:
                        raise ValueError(
                            "Attribute 'valid_values' of the argument '{}' must be either two or three long and of the form (min, max, [extra_values]).".format(
                                syntax_element["name"]
                            )
                        )
                else:
                    raise TypeError(
                        "Attribute 'valid_values' of the argument '{}' must be a tuple of the form (min, max, [extra_values]).".format(
                            syntax_element["name"]
                        )
                    )
            else:
                raise ValueError(
                    "'{}' is not a valid value for the attribute 'type' for the argument ''.".format(
                        syntax_element["type"], syntax_element["name"]
                    )
                )
...
def argument_parser(command_string, command_syntax):
    ...
    for element_index, syntax_element in enumerate(command_syntax):
        try:
            ...
            if syntax_element["required"]:
                if "error_not_specified" in syntax_element:
                    print(
                        syntax_element["error_not_specified"].format(*command_arguments)
                    )
                else:
                    print(
                        "The argument '{}' is not specified.".format(
                            syntax_element["name"]
                        )
                    )
                return
        ...
        if syntax_element["type"] == "string":
            if (
                ...
                if argument_text in syntax_element["valid_values"]:
                    ...
                    if "error_not_valid_value" in syntax_element:
                        print(
                            syntax_element["error_not_valid_value"].format(
                                *command_arguments
                            )
                        )
                    else:
                        ...
                        print(
                            "'{}' is not a valid value for '{}' must be one of: {}.".format(
                                argument_text,
                                syntax_element["name"],
                                valid_values_string,
                            )
            ...
            if (
                len(syntax_element["valid_values"]) == 3
                and argument_text in syntax_element["valid_values"][2]
            ):
                pass
            else:
                try:
                    ...
                    if "error_not_number" in syntax_element:
                        print(
                            syntax_element["error_not_number"].format(
                                *command_arguments
                            )
                        )
                    else:
                        print(
                            "The argument '{}' must be a number.".format(
                                syntax_element["name"]
                            )
                        )
                    return
                    ...
                    if syntax_element["valid_values"][0] <= argument_text:
                        if argument_text <= syntax_element["valid_values"][1]:
                            ...
                            if "error_too_big" in syntax_element:
                                print(
                                    syntax_element["error_too_big"].format(
                                        *command_arguments
                                    )
                                )
                            else:
                                print(
                                    "'{}' is bigger than {}.".format(
                                        syntax_element["name"],
                                        syntax_element["valid_values"][1],
                                    )
                                )
                        ...
                        if "error_too_small" in syntax_element:
                            print(
                                syntax_element["error_too_small"].format(
                                    *command_arguments
                                )
                            )
                        else:
