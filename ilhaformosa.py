...
class IlhaFormosa(cmd.Cmd):
        ...
        if args is None:
            ...
            if product == "food":
                if quantity in ["max", "all"]:
                    quantity = 10
                price = quantity * random_price(
                    5.5, sd=1
                )  # TODO: change random_price to accept a quantity variable.
                if player.cash > price:
                    ...
                    return
            if product == "ship" or (
                player.location.for_sale_ship is not None
                and product == player.location.for_sale_ship.type
            ):
                if "shipyard" in player.location.buildings:
                    if player.location.for_sale_ship is None:
                        ...
                        if player.cash >= price:
                            print(
                                "You buy {} for {}.".format(
                                    player.location.for_sale_ship.nickname, money(price)
                                )
                            )
                            player.cash -= price
                ...
                if empty_space == 0:
                    ...
                    return
                if quantity in ["max", "all"]:
                    max_quantity_cash = math.floor(
                        player.cash / player.location.local_prices[product]
                    )
                ...
                if quantity <= empty_space:
                    if price <= player.cash:
                        print(
                            "You buy {} of {} for {}.".format(
                                weight(quantity), product, money(price)
                            )
                        )
                        player.cash -= price
                        player.set_cargo_quantity(
                            product, player.get_cargo_weight(product) + quantity
                        )
                        return
                    else:
                        print(
                            "You do not have enough money to buy {} of {}.".format(
                    ...
                    print(
                        "You do not have enough empty space on your fleet to buy {} of {}. You have room for {} of cargo.".format(
                            weight(quantity), product, weight(empty_space)
                        )
                    )
                    return
            else:
    ...
    def complete_buy(self, text, line, begidx, endidx):
        ...
        if len(args) == 2:
            return [
                k
                for k in ["food", "ship"] + cargo.types
                if k.startswith(format_arg(text))
            ]
        else:
            return []
    ...
    def do_calendar(self, input_arguments):
        ...
        if args is None:
            ...
            if len(args) == 0:
                ...
                print(
                    "In {} days it will be {}.".format(
                        args["days"], day_to_date(player.day + args["days"])
                    )
                )
                return
    ...
    ):  # TODO: Add cargo tetris? Add more detailed cargo management?
    ...
    def do_fleet(self, input_arguments):  # TODO: Turn this into a flexitable
        ...
        if args is None:
            ...
            if len(args) > 0:
                ...
                print(
                    "Your fleet has {} ship{}.".format(
                        len(player.fleet), "s" * is_plural
                    )
                )
                print("")
    ...
    ):  # TODO: Add a more complex regex matching algorithm to return all matches
        """Tab completion for the fleet command."""
        return [
            format_arg(k.nickname)
            for k in player.fleet
            if format_arg(k.nickname).startswith(format_arg(text))
        ]
    ...
    def do_help(self, input_arguments):
        ...
        if args is None:
            ...
            if len(args) == 0:
                widest_command = max(
                    [len(command_name) for command_name in all_commands]
                )
                print("All commands:")
                n_commands_per_line = math.floor(90 / (widest_command + 1))
                for line_of_commands in [
                    all_commands[i : i + n_commands_per_line]
                    for i in range(0, len(all_commands), n_commands_per_line)
                ]:
                    print(
                        " ".join(
                            [
                                align_text(command_name, widest_command, "l")
                                for command_name in line_of_commands
                            ]
                        )
                    )
                ...
                def show_command_help(command_name, _self=self):
                    ...
                    if (
                        command_name in command_syntax
                        and len(command_syntax[command_name]) > 0
                    ):
                        ...
                        for argument in selected_command_syntax:
                            if argument["required"]:
                                ...
                                brackets = "[]"
                            command_syntax_string += (
                                " " + brackets[0] + argument["name"] + brackets[1]
                            )
                    print(
                        command_syntax_string
                        + " - "
                        + getattr(_self, "do_" + command_name).__doc__
                    )
                    return
    ...
    def do_map(
        ...
        if args is None:
            ...
            print(
                "You are in {}. Your map has these ports on it:".format(
            ...
            for port_id in all_port_ids:
                if player.location.id == port_id:
                    ...
                    print(
                        "{} ({} nautical miles)".format(
                            port_id_to_name(port_id),
                            ports_distances[player.location.name][
                                port_id_to_name(port_id)
                            ],
                        )
                    )
    ...
    def complete_options(self, text, line, begidx, endidx):
        ...
        if len(args) == 1:
            return [
                option_name for option_name in {**options.choices, **{"default": []}}
            ]
        elif len(args) == 2:
            return [
                option_name
                for option_name in {**options.choices, **{"default": []}}
                if option_name.startswith(text)
            ]
        elif len(args) == 3 and args[1] in options.choices:
            option_name = args[1]
            return [
                option_choice
                for option_choice in options.choices[option_name] + ["default"]
                if option_choice.startswith(text)
            ]
        else:
            return []
    ...
    def do_sail(self, input_arguments):
        ...
        if args is None:
            ...
            if (
                player.location == destination_port
                ...
                if player.get_cargo_weight() > player.get_cargo_capacity():
                    print(
                        "Your ships are too full to sail. You have {} but your ships' capacity is {}. Sell some cargo or buy a ship to continue.".format(
                            weight(player.get_cargo_weight()),
                            weight(player.get_cargo_capacity()),
                        )
                    )
                else:
                    ...
                    # while travelling
                    journey_distance = ports_distances[departing_port.name][
                        arriving_port.name
                    ]
                    journey_speed = 8  # TODO: Change this to top speed.
                    journey_time = int(math.ceil(journey_distance / 8) / 24)
                    cargo.global_values = cargo.randomise_values(
                        cargo.global_values, journey_time
                    )
                    player.day += journey_time
                    print(
                        "You sail {} nautical miles at {} knots for {} day{}.".format(
                            journey_distance,
                            journey_speed,
                            math.floor(journey_time),
                            (math.floor(journey_time) > 1) * "s",
                        )
                    )
                    ...
                    if "shipyard" in arriving_port.buildings:
                        ...
                        arriving_port.for_sale_ship_price = random_price(
                            arriving_port.for_sale_ship.value, base=100
                        )
                    # market
                    arriving_port.local_values = cargo.randomise_values(
                        arriving_port.local_values, since_last_visit
                    )
                    arriving_port.local_prices = cargo.calculate_prices(
                        arriving_port.local_values
                    )
                    player.location = arriving_port
                    print(
                        "You land in {} on {}.".format(
                            arriving_port.name, day_to_date(player.day)
                        )
                    )
                    return
    ...
    def complete_sail(self, text, line, begidx, endidx):
        ...
        return [
            port
            for port in world
            if (port.startswith(text) and port != player.location.id)
        ]
    ...
    def default(self, input_arguments):
        ...
        print(
            "'{}' is not a known command. Type ? for a list of commands.".format(
                command_name
            )
        )
